...
func TestPublishArticle(t *testing.T) {
	Convey("PUT /api/v1/admin/articles/{id}/publish", t, func() {
		var svcErr error

		svc := &mockService{
			publishArticle: func(_ context.Context, _, _ string) error {
				return svcErr
			},
		}

		f := newHTTPFixture(svc, http.MethodPut, "/api/v1/admin/articles/11111111-1111-1111-1111-111111111111/publish")
		...
		Convey("Valid request → 200 with message", func() {
			...
			So(body["message"], ShouldEqual, "article was successfully published")
		})
		...
		Convey("invalid ArticleID", func() {
			f := newHTTPFixture(svc, http.MethodPut, "/api/v1/admin/articles/---/publish")
			mux, newReq := f.mux, f.newReq
