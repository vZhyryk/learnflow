...
func TestPublishCourse(t *testing.T) {
	Convey("PUT /api/v1/admin/courses/{id}/publish", t, func() {
		var svcErr error
		...
		f := newHTTPFixture(svc, http.MethodPut, "/api/v1/admin/courses/11111111-1111-1111-1111-111111111111/publish")
		...
		Convey("Valid request → 200 with message", func() {
			...
			So(body["message"], ShouldEqual, "course was successfully published")
		})
		...
		Convey("invalid courseID", func() {
			f := newHTTPFixture(svc, http.MethodPut, "/api/v1/admin/courses/---/publish")
			mux, newReq := f.mux, f.newReq
