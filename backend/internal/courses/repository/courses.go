...
func (rep *Repository) PublishCourse(ctx context.Context, courseID string) error {
	tag, err := rep.queryRunner(ctx).Exec(ctx, publishCourseSQL, courseID)

	if err != nil {
		return fmt.Errorf("repository.PublishCourse: %w", err)
	}
...
func (rep *Repository) ArchiveCourse(ctx context.Context, courseID string) error {
	tag, err := rep.queryRunner(ctx).Exec(ctx, archiveCourseSQL, courseID)
	if err != nil {
		return fmt.Errorf("repository.ArchiveCourse: %w", err)
	}
...
func (rep *Repository) DeleteCourse(ctx context.Context, courseID string) error {
	tag, err := rep.queryRunner(ctx).Exec(ctx, deleteCourseSQL, courseID)
	if err != nil {
		return fmt.Errorf("repository.DeleteCourse: %w", err)
	}
...
func (rep *Repository) GetAllPublishedCourses(ctx context.Context) ([]*coursedomain.Course, error) {
	return rep.getAndParseCourses(ctx, getAllPublishedCoursesSQL, "GetAllPublishedCourses")
}

// GetAllDraftCourses returns every non-deleted draft course.
func (rep *Repository) GetAllDraftCourses(ctx context.Context) ([]*coursedomain.Course, error) {
	return rep.getAndParseCourses(ctx, getAllDraftCoursesSQL, "GetAllDraftCourses")
}

// GetAllArchivedCourses returns every archived course, including soft-deleted ones.
func (rep *Repository) GetAllArchivedCourses(ctx context.Context) ([]*coursedomain.Course, error) {
	return rep.getAndParseCourses(ctx, getAllArchivedCoursesSQL, "GetAllArchivedCourses")
}

// GetAllCourses returns every course regardless of status, including soft-deleted ones.
func (rep *Repository) GetAllCourses(ctx context.Context) ([]*coursedomain.Course, error) {
	return rep.getAndParseCourses(ctx, getAllCoursesSQL, "GetAllCourses")
}
