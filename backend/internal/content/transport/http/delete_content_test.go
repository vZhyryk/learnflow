...
func TestDeleteContentItem(t *testing.T) {
	Convey("DELETE /api/v1/admin/content/{id}", t, func() {
		var svcErr error

		svc := &mockService{
			deleteContentItem: func(_ context.Context, _, _ string) error {
				return svcErr
			},
		}

		f := newHTTPFixture(svc, http.MethodDelete, "/api/v1/admin/content/11111111-1111-1111-1111-111111111111")
		...
		Convey("Valid request → 200 with message", func() {
			...
			So(body["message"], ShouldEqual, "content was successfully deleted")
		})
		...
		Convey("invalid contentItemID", func() {
			f := newHTTPFixture(svc, http.MethodDelete, "/api/v1/admin/content/---")
			mux, newReq := f.mux, f.newReq
