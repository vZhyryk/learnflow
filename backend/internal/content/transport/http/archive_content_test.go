...
func TestArchiveContentItem(t *testing.T) {
	Convey("PUT /api/v1/admin/content/{id}/archive", t, func() {
		var svcErr error

		svc := &mockService{
			archiveContentItem: func(_ context.Context, _, _ string) error {
				return svcErr
			},
		}

		f := newHTTPFixture(svc, http.MethodPut, "/api/v1/admin/content/11111111-1111-1111-1111-111111111111/archive")
		...
		Convey("invalid contentItemID", func() {
			invalidF := newHTTPFixture(svc, http.MethodPut, "/api/v1/admin/content/---/archive")
		...
		Convey("Valid request → 200 with message", func() {
			...
			So(body["message"], ShouldEqual, "content was successfully archived")
		})
