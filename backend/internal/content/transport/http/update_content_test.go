...
func TestUpdateContentItem(t *testing.T) {
	Convey("PUT /api/v1/admin/content", t, func() {
		var svcErr error
		validBody := `{"id":"11111111-1111-1111-1111-111111111111"}`

		svc := &mockService{
			updateContentItem: func(_ context.Context, _ contentdomain.UpdateContentItemRequest, _ string) error {
				return svcErr
			},
		}

		f := newHTTPFixture(svc, http.MethodPut, "/api/v1/admin/content")
		...
		Convey("Missing content id fails validation → 400", func() {
			w := testutil.ServeHTTP(mux, withUser(newReq(`{}`, nil)))
			So(w.Code, ShouldEqual, http.StatusBadRequest)
		})
