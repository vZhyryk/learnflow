...
func TestPublishContentItem(t *testing.T) {
	Convey("PUT /api/v1/admin/content/{id}/publish", t, func() {
		var svcErr error

		svc := &mockService{
			publishContentItem: func(_ context.Context, _, _ string) error {
				return svcErr
			},
		}

		f := newHTTPFixture(svc, http.MethodPut, "/api/v1/admin/content/11111111-1111-1111-1111-111111111111/publish")
		...
		Convey("Valid request → 200 with message", func() {
			...
			So(body["message"], ShouldEqual, "content was successfully published")
		})
		...
		Convey("invalid contentItemID", func() {
			f := newHTTPFixture(svc, http.MethodPut, "/api/v1/admin/content/---/publish")
			mux, newReq := f.mux, f.newReq
