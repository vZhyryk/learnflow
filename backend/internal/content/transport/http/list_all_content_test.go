package contenthttp_test

import (
	"context"
	contentdomain "learnflow_backend/internal/content/domain"
	"learnflow_backend/internal/shared/pagination"
	"learnflow_backend/internal/shared/testutil"
	"net/http"
	"testing"

	. "github.com/smartystreets/goconvey/convey"
)

func TestListAllContent(t *testing.T) {
	Convey("GET /api/v1/admin/content", t, func() {
		var svcErr error

		svc := &mockService{
			getAllContentItems: func(_ context.Context, _ contentdomain.ContentItemStatus, _ pagination.Params) ([]*contentdomain.ContentItem, error) {
				return []*contentdomain.ContentItem{}, svcErr
			},
		}

		f := newHTTPFixture(svc, http.MethodGet, "/api/v1/admin/content")
		mux, newReq := f.mux, f.newReq

		Convey("No user in context → panics (middleware invariant violated)", func() {
			So(func() {
				testutil.ServeHTTP(mux, newReq("", nil))
			}, ShouldPanic)
		})

		Convey("invalid status", func() {
			urlParam := map[string]string{
				"status": "wrong_status",
			}
			w := testutil.ServeHTTP(mux, withUser(newReq("", urlParam)))
			So(w.Code, ShouldEqual, http.StatusUnprocessableEntity)
		})

		Convey("Unexpected service error → 500", func() {
			svcErr = testutil.ErrDBUnexpected
			w := testutil.ServeHTTP(mux, withUser(newReq("", nil)))
			So(w.Code, ShouldEqual, http.StatusInternalServerError)
		})

		Convey("Valid request → 200 with message - empty status", func() {
			w := testutil.ServeHTTP(mux, withUser(newReq("", nil)))
			So(w.Code, ShouldEqual, http.StatusOK)
			body := decodeBody(t, w.Body.Bytes())
			So(body["content_item_list"], ShouldNotBeNil)
		})

		Convey("Valid request → 200 with message - valid status", func() {
			urlParam := map[string]string{
				"status": "draft",
			}
			w := testutil.ServeHTTP(mux, withUser(newReq("", urlParam)))
			So(w.Code, ShouldEqual, http.StatusOK)
			body := decodeBody(t, w.Body.Bytes())
			So(body["content_item_list"], ShouldNotBeNil)
		})

		Convey("Valid request and the success response write fails → does not panic", func() {
			So(func() {
				mux.ServeHTTP(&errWriter{}, withUser(newReq("", nil)))
			}, ShouldNotPanic)
		})
	})
}
