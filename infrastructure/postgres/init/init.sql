...
-- Reference snapshot only — canonical schema lives in backend/migrations/
-- Tables: users, user_profiles, email_verification_tokens, password_reset_tokens,
--         email_change_tokens, courses, content_items, course_content_items,
--         course_reviews, content_reviews, payments, payment_line_items, refunds,
--         user_course_access, user_course_progress, user_content_progress,
--         user_video_engagement, user_document_engagement, user_content_access,
--         user_notes, consultation_slots, consultation_briefs, consultation_bookings,
...
-- Lookup: "which payment covers this resource?" (e.g. find payment for a given course purchase)
...
CREATE TABLE user_notes (
    user_id       uuid        NOT NULL REFERENCES users(id) ON DELETE RESTRICT,
    id            uuid        PRIMARY KEY DEFAULT gen_random_uuid(),
    -- Optional short label shown in note list; full content is in body
...
-- date + start_time are stored in the platform's local timezone (Europe/Warsaw).
-- Application layer is responsible for timezone-aware display and DST handling.
